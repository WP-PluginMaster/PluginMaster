...
<div class="wrap">
    ...
    <div>
        ...
        <blockquote>
            « PluginMaster is an Application development framework for WordPress. It changes the flavor of plugin
            development.»
        </blockquote>
    ...
    <div id="DatabaseMigrationSystem">
            <h4 class="mt-5 mb-5"> 1. <a href="#DatabaseMigrationSystem">Database Migration System</a></h4>
            <p> Migrations are typically paired with the <b>PluginMaster</b> schema builder to build your
                application's database schema. When plugin activates, all migration migrate to the database and when
                deactivate plugin, all table will be deleted. You just need to define the migration file. </p>
            ...
            <ol>
                <li>First for sequence maintain for foreign key and second for the
                    table name.
                </li>
                <li>Second part must be the same as table name .</li>
            </ol>
            <blockquote>
                Also class name will be the same as the table name.<br> File name like : <code>1_demo_users.php</code>.
                This means,
                <code>1</code> is for sequence maintain and <code>demo_users</code> is the table name and also class
            ...
            <pre><code>use App\system\schema\Schema;
class demo_users
{
    public function up()
    {
      return Schema::create('demo_users', function (Schema $column) {
            ...
            <ol>
                ...
                <li><code> default($value)</code> : DEFAULT 'VALUE' (IF $VALUE == 'CURRENT_TIMESTAMP' OR
                    'current_timestamp' then set <code>CURRENT_TIMESTAMP </code></li>
                ...
                <li><code> on($reference)</code> : $reference means table.column. check following example.
                    ...
                    <pre class="mt-2"><code>$column->integer('user_id')->unsigned();
  $column->foreign('user_id')->on('demo_users.id'); </code></pre>
     ...
     <div id="SimpleSideMenuDeclaration">

            <h4>2. <a href="#SimpleSideMenuDeclaration">Simple Side Menu Declaration</a> </h4>
    ...
    $sidenav->sub('Dashboard', ["title" => "Dashboard", "as" => 'DemoController@sub']);
            ...
            <ol>
                ...
                <li>Two : menu options . Its type must be an array. <br>
                    <b>Mandatory index:</b> A. icon (dashboard icon), B. as ( controller and method must be with
                    <code>@</code> sign) <br>
                    ...
                    <b>Optional index:</b> position (default is 500), removeFirstSubmenu ( for delete first submenu,
                    default false) <br>
                </li>
     ...
     <div id="EasyRestAPIRouteDeclaration">

            <h4 class="mt-5">3. <a href="#EasyRestAPIRouteDeclaration">Easy Rest API Route Declaration</a></h4>
            ...
            <pre><code>$route->get('dashboard/{id?}', 'DemoController@dashboard');
$route->post('add-note', 'DemoController@addNote');  </code> </pre>

            <span><code>DemoController</code> is controller name and <code>dashboard</code> is method name</span>
            <p class="mt-2"> Currently PluginMaster support Two Methods for rest route : <code>GET</code>,
                <code>POST</code> </b></p>
            <b> Parameters for <code>get()</code> and <code>post()</code> methods:</b>
            <p>First Parameter for Route: <b>Route Name</b></p>
            <ol>
                <li>Dynamic Parameter : <code>{parameter_name}</code></li>
                <li>Dynamic Optional Parameter : <code>{parameter_name?} </code></li>
                <li> Access Dynamic Param in Controller method : set parameter like : <code>function
                        dashboard($variable)</code> then <code>$variable['id'] </code></li>
     ...
     <div id="QueryBuilder">
            ...
            <h4 class="mt-5">4. <a href="#QueryBuilder">DB Query Builder</a></h4>
            <p>PluginMaster's database query builder provides a convenient, fluent interface to running
    ...
    $query->table('demo_notes')
     ...
     <div id="SimpleEnqueueDeclaration">

            <h4 class="mt-5">5.  <a href="#SimpleEnqueueDeclaration">Simple Enqueue Declaration</a></h4>
            ...
            <p><b>Version:</b> Enqueue version maintain by <code>enqueue_version</code> of config.php. It's located in
                <code>app/config/config.php</code></p>
            <p class="mt-3"><b>Functions</b></p>
            <ol>
                <li><code> headerScript($path) : </code> Example :
                    <code>$enqueue->headerScript('assets/js/index.js');</code>
                </li>
                <li><code> headerScriptCdn($path) : </code> Example : <code>$enqueue->headerScriptCdn('http://alemran.me/js/index.js');</code>
                </li>
                <li><code> footerScript($path) : </code> Example :
                    <code>$enqueue->footerScript('assets/js/index.js');</code>
                </li>
                <li><code> footerScriptCdn($path) : </code> Example : <code>$enqueue->footerScriptCdn('http://alemran.me/js/index.js');</code>
                </li>
                <li><code> style($path) : </code> Example : <code>$enqueue->style('assets/css/style.css');</code>
                </li>
                <li><code> styleCdn($path) : </code> Example : <code>$enqueue->style('assets/js/index.css');</code>
                </li>
                <li><code> csrfToken($handler, $objectName): For CSRF protected Rest API </code> Example :
...
$enqueue->csrfToken('DemoScriptIndex','corsData');
     ...
     <div id="RequestHandlingSystem" >
        ...
        <h4 class="mt-5"> 6. <a href="#RequestHandlingSystem">Request Handling System</a></h4>
    ...
    <div id="Validator">
        ...
        <h4 class="mt-5"> 7.<a href="#Validator"> Validator</a></h4>
 ...
 if ($validator->fails()) {
     $errors =  $validator->errors();
        ...
        <p>In view file</p>
        <pre class="mt-3"><code>              if ( count(formErrors()) ) :
    ...
    <div id="BuildinVueJSConfiguration">

        <h4 class="mt-5"> 8. <a href="#BuildinVueJSConfiguration">Build in Vue JS Configuration </a></h4>
        ...
        <p><b> Run : <code>npm run hot</code></b>. (for on-change build and reload) (default hot mode(webpack
            dev server) listen from port 8080. if you run this command , you must declare <code>$enqueue->hotScript('file_name.js') </code>
            in enqueue .</p>
 ...
 const buildConfig = [
    ...
    <div id="SessionHandler">
        ...
        <h4 class="mt-5"> 8. <a href="#SessionHandler"> Session Handler </a></h4>
